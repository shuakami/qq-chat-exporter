...
export class ExcelExporter extends BaseExporter {
    private readonly excelOptions: ExcelFormatOptions;
     ...
     * 添加统计信息工作表
     */
    private addStatisticsSheet(workbook: XLSX.WorkBook, messages: CleanMessage[]): void {
     ...
     * 添加发送者统计工作表
     */
    private addSenderStatsSheet(workbook: XLSX.WorkBook, messages: CleanMessage[]): void {
        const parser = new SimpleMessageParser();
        const stats = parser.calculateStatistics(messages);
     ...
     * 提取消息的文本内容
     */
    private extractTextContent(msg: CleanMessage): string {
