...
export class StickerPackExporter {
     ...
     * 添加导出记录
     */
    private addRecord(record: ExportRecord): void {
     ...
     * 获取导出记录
     */
    getExportRecords(limit: number = 50): ExportRecord[] {
     ...
     */
    async getStickerPacks(types?: StickerPackType[]): Promise<StickerPackInfo[]> {
        try {
            const packs: StickerPackInfo[] = [];
    ...
    private async getMarketEmoticonPacks(packs: StickerPackInfo[]): Promise<void> {
        try {
            ...
            for (const tab of tabList) {
                ...
                try {
                    const jsonResult: any = await msgService.fetchMarketEmotionJsonFile(epId);
                ...
                try {
                    const imagesResult: any = await msgService.fetchMarketEmoticonFaceImages(epId);
                ...
                try {
                    const aioResult: any = await msgService.fetchMarketEmoticonAioImage(epId);
    ...
    private parseMarketPackJsonData(packData: any, _epId: string, packName: string): StickerInfo[] {
        const stickers: StickerInfo[] = [];
    ...
    private parseMarketPackInfo(packInfo: any, _epId: string, packName: string): StickerInfo[] {
        const stickers: StickerInfo[] = [];
            ...
            if (fs.existsSync(faceConfigPath)) {
                ...
                if (faceConfig.sysface && Array.isArray(faceConfig.sysface)) {
                    ...
                    for (const [packName, faces] of packMap.entries()) {
                        const stickers: StickerInfo[] = faces.map(face => ({
     ...
     * 导出所有表情包
     */
    async exportAllStickerPacks(): Promise<ExportResult> {
        try {
            const packs = await this.getStickerPacks();
            ...
            if (favoriteEmojis.length > 0) {
                const favoriteData = {
                    ...
                    stickers: favoriteEmojis.flatMap(pack =>
                        pack.stickers.map(s => ({
                            id: s.stickerId,
                            name: s.name,
                            url: s.path,
