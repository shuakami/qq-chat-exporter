...
async fn call_group_history_once(
    ...
    if cooldown_until > now {
        return Err(format!(
            "QQ 历史接口刚刚发生异常，已停止继续请求；请等待约 {} 秒后再试",
    ...
    let _permit = group_history_gate()
        .acquire()
        .await
        .map_err(|_| "QQ 历史接口串行锁已关闭".to_string())?;
    ...
    match tokio::time::timeout(Duration::from_millis(SAFE_GROUP_CALL_TIMEOUT_MS), call).await {
        ...
        Ok(Err(error)) => {
            ...
            Err(format!(
                "QQ 历史接口调用失败，已停止且不会自动重试，避免 Worker 重复崩溃: {error}"
            ))
        }
        Err(_) => {
            open_group_history_circuit();
            Err(format!(
                "QQ 历史接口等待超过 {} 秒，已停止且不会自动重试；旧原生请求可能仍在 QQ 内部执行",
...
async fn fetch_safe_group_history_to_spool(
    ...
    tokio::time::timeout(Duration::from_secs(10), state.napcat.self_info())
        .await
        .map_err(|_| "读取 QQ 登录状态超时，未开始历史消息请求".to_string())?
        .map_err(|error| format!("QQ 当前未处于可用登录状态，未开始历史消息请求: {error}"))?;
    ...
    // first-view 接口只取 1 条用于获得最新 msgSeq；不能把它当作批量历史接口使用。
    ...
    let latest_seq = bootstrap_messages
        ...
        .max()
        .ok_or_else(|| "QQ 返回的最新消息缺少 msgSeq，无法安全进行序列分页".to_string())?;

    let mut total_written = 0usize;
    let mut page_count = 0usize;
    let mut anchor_seq = latest_seq;
    ...
    loop {
        ...
        let result = call_group_history_once(
            ...
            json!([peer.clone(), anchor_seq.to_string(), batch_size, true, true]),
        )
        .await?;
        ...
        let minimum_seq = raw_messages
            ...
            .filter_map(raw_message_seq)
            .min()
            .ok_or_else(|| format!("第 {page_count} 批消息均缺少 msgSeq，已停止安全分页"))?;
        if minimum_seq >= anchor_seq {
            return Err(format!(
                "QQ 序列游标没有向更早消息移动（anchor={anchor_seq}, min={minimum_seq}），已停止以避免重复请求"
            ));
        }
        ...
        let message = format!(
            "安全序列分页中 · 第 {page_count} 批 · 已写入 {total_written} 条 · 当前批次 {batch_size} · {elapsed} ms"
        );
        progress(
            state,
            task_id,
        ...
        anchor_seq = minimum_seq.saturating_sub(1);
        successes_at_size += 1;
