 ...
 * Original file: /npm/@paper-design/shaders@0.0.77/dist/index.js
 *
...
}`;function ke(o){return"paperShaderMount"in o}function Ie(){const o=navigator.userAgent.toLowerCase();return o.includes("safari")&&!o.includes("chrome")&&!o.includes("android")}function Re(o){const e=visualViewport?.scale??1,a=visualViewport?.width??window.innerWidth,i=window.innerWidth-o.documentElement.clientWidth,f=e*a+i,c=outerWidth/f,n=Math.round(100*c);return n%5===0?n/100:n===33?1/3:n===67?2/3:n===133?4/3:c}const Fe={fit:"contain",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},Me={fit:"none",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},Ee={none:0,contain:1,cover:2},w=`
...
`,se={maxColorCount:10},Oe=`#version 300 es
precision mediump float;
...
`,$={maxColorCount:10,maxNoiseIterations:8},De=`#version 300 es
precision mediump float;
...
vec2 fbm(vec2 n0, vec2 n1) {
  vec2 total = vec2(0.0);
  float amplitude = .4;
  for (int i = 0; i < ${$.maxNoiseIterations}; i++) {
...
void main() {
  ...
  for (int i = ${$.maxColorCount} - 2; i >= 0; i--) {
...
`,le={maxColorCount:10},Te=`#version 300 es
precision mediump float;
...
`,Pe={circle:0,diamond:1,square:2,triangle:3},ne={maxColorCount:10},Ne=`#version 300 es
precision mediump float;
...
`,ee={maxColorCount:8,maxBallsCount:20},We=`#version 300 es
precision mediump float;
...
void main() {
  ...
  for (int i = 0; i < ${ee.maxBallsCount}; i++) {
    if (i >= int(ceil(u_count))) break;

    float idxFract = float(i) / float(${ee.maxBallsCount});
...
`,ce={maxColorCount:5},Le=`#version 300 es
precision mediump float;
...
`,ue={maxColorCount:10},Xe=`#version 300 es
precision mediump float;
...
`,Ke={checks:0,stripes:1,edge:2},fe={maxColorCount:5},je=`#version 300 es
precision mediump float;
...
`,me={maxColorCount:10},qe=`#version 300 es
precision mediump float;
...
`,$e={simplex:1,warp:2,dots:3,wave:4,ripple:5,swirl:6,sphere:7},eo={random:1,"2x2":2,"4x4":3,"8x8":4},pe={maxColorCount:7},oo=`#version 300 es
...
`,to={wave:1,dots:2,truchet:3,corners:4,ripple:5,blob:6,sphere:7},oe={maxColorCount:5,maxSpots:4},ao=`#version 300 es
precision lowp float;
...
void main() {
  ...
  for (int colorIdx = 0; colorIdx < ${oe.maxColorCount}; colorIdx++) {
    ...
    for (int spotIdx = 0; spotIdx < ${oe.maxSpots}; spotIdx++) {
  ...
  fragColor = vec4(color, opacity);
}`,io={auto:0,square:1},te={maxColorCount:7},ro=`#version 300 es
precision lowp float;
...
void main() {
  ...
  vec4 premultipliedColors[${te.maxColorCount}];
  for (int i = 0; i < ${te.maxColorCount}; i++) {
...
`,de={maxColorCount:10},so=`#version 300 es
precision mediump float;
...
`,fo={lines:1,linesIrregular:2,wave:3,zigzag:4,pattern:5},mo={prism:1,lens:2,contour:3,cascade:4,flat:5},po=`#version 300 es
precision mediump float;
...
`,ve={maxColorCount:10},go=`#version 300 es
...
`;function vo(o){const e=document.createElement("canvas"),a=1e3;return new Promise((i,f)=>{const c=new Image;c.crossOrigin="anonymous",c.addEventListener("load",()=>{(typeof o=="string"?o.endsWith(".svg"):o.type==="image/svg+xml")&&(c.width=a,c.height=a);const n=c.naturalWidth/c.naturalHeight,t=Math.floor(a*.15),m=Math.ceil(t*2.5);let s=a,u=a;n>1?u=Math.floor(a/n):s=Math.floor(a*n),e.width=s+2*m,e.height=u+2*m;const p=e.getContext("2d",{willReadFrequently:!0});if(!p)throw new Error("Failed to get canvas 2d context");p.fillStyle="white",p.fillRect(0,0,e.width,e.height),p.drawImage(c,m,m,s,u);const{width:b,height:k}=e,r=p.getImageData(0,0,b,k).data,l=b*k,v=new Uint8ClampedArray(l);for(let _=0;_<l;_++){const M=_*4,Y=r[M]??0,T=r[M+1]??0,H=r[M+2]??0;v[_]=.299*Y+.587*T+.114*H|0}const C=t,S=Math.max(1,Math.round(.12*t)),B=5,g=he(v,b,k,C,3),A=he(v,b,k,S,3),R=he(v,b,k,B,1),P=p.createImageData(b,k),Q=P.data;for(let _=0;_<l;_++){const M=_*4;Q[M]=R[_]??0,Q[M+1]=g[_]??0,Q[M+2]=A[_]??0,Q[M+3]=255}p.putImageData(P,0,0),e.toBlob(_=>{if(!_){f(new Error("Failed to create PNG blob"));return}i({blob:_})},"image/png")}),c.addEventListener("error",()=>{f(new Error("Failed to load image"))}),c.src=typeof o=="string"?o:URL.createObjectURL(o)})}function Ae(o,e,a,i){if(i<=0)return o.slice();const f=new Uint8ClampedArray(e*a),c=new Uint32Array(e*a);for(let n=0;n<a;n++){let t=0;for(let m=0;m<e;m++){const s=n*e+m,u=o[s]??0;t+=u,c[s]=t+(n>0?c[s-e]??0:0)}}for(let n=0;n<a;n++){const t=Math.max(0,n-i),m=Math.min(a-1,n+i);for(let s=0;s<e;s++){const u=Math.max(0,s-i),p=Math.min(e-1,s+i),b=m*e+p,k=m*e+(u-1),y=(t-1)*e+p,r=(t-1)*e+(u-1),l=c[b]??0,v=u>0?c[k]??0:0,C=t>0?c[y]??0:0,S=u>0&&t>0?c[r]??0:0,B=l-v-C+S,g=(p-u+1)*(m-t+1);f[n*e+s]=Math.round(B/g)}}return f}function he(o,e,a,i,f){if(i<=0||f<=1)return Ae(o,e,a,i);let c=o,n=o;for(let t=0;t<f;t++)n=Ae(c,e,a,i),c=n;return n}const ho=`#version 300 es
precision mediump float;
...
`,be={workingSize:512,iterations:40};function xo(o){const e=document.createElement("canvas"),a=e.getContext("2d"),i=typeof o=="string"&&o.startsWith("blob:");return new Promise((f,c)=>{if(!o||!a){c(new Error("Invalid file or canvas context"));return}const n=i&&fetch(o).then(m=>m.headers.get("Content-Type")),t=new Image;t.crossOrigin="anonymous",performance.now(),t.onload=async()=>{let m;const s=await n;s?m=s==="image/svg+xml":typeof o=="string"?m=o.endsWith(".svg")||o.startsWith("data:image/svg+xml"):m=o.type==="image/svg+xml";let u=t.width||t.naturalWidth,p=t.height||t.naturalHeight;if(m){const h=u/p;u>p?(u=4096,p=4096/h):(p=4096,u=4096*h),t.width=u,t.height=p}const b=Math.min(u,p),y=be.workingSize/b,r=Math.round(u*y),l=Math.round(p*y);e.width=u,e.height=p;const v=document.createElement("canvas");v.width=r,v.height=l;const C=v.getContext("2d");C.drawImage(t,0,0,r,l),performance.now();const B=C.getImageData(0,0,r,l).data,g=new Uint8Array(r*l),A=new Uint8Array(r*l);for(let d=0,h=0;d<B.length;d+=4,h++){const F=B[d+3]===0?0:1;g[h]=F}const R=[],P=[];for(let d=0;d<l;d++)for(let h=0;h<r;h++){const U=d*r+h;if(!g[U])continue;let F=!1;h===0||h===r-1||d===0||d===l-1?F=!0:F=!g[U-1]||!g[U+1]||!g[U-r]||!g[U+r]||!g[U-r-1]||!g[U-r+1]||!g[U+r-1]||!g[U+r+1],F?(A[U]=1,R.push(U)):P.push(U)}const Q=_o(g,A,new Uint32Array(P),new Uint32Array(R),r,l);performance.now();const _=yo(Q,g,A,r,l);let M=0,Y;for(let d=0;d<P.length;d++){const h=P[d];_[h]>M&&(M=_[h])}const T=document.createElement("canvas");T.width=r,T.height=l;const H=T.getContext("2d"),D=H.createImageData(r,l);for(let d=0;d<l;d++)for(let h=0;h<r;h++){const U=d*r+h,F=U*4;if(!g[U])D.data[F]=255,D.data[F+1]=255,D.data[F+2]=255,D.data[F+3]=0;else{const K=255*(1-_[U]/M);D.data[F]=K,D.data[F+1]=K,D.data[F+2]=K,D.data[F+3]=255}}H.putImageData(D,0,0),a.imageSmoothingEnabled=!0,a.imageSmoothingQuality="high",a.drawImage(T,0,0,r,l,0,0,u,p);const E=a.getImageData(0,0,u,p),X=document.createElement("canvas");X.width=u,X.height=p;const L=X.getContext("2d");L.drawImage(t,0,0,u,p);const J=L.getImageData(0,0,u,p);for(let d=0;d<E.data.length;d+=4){const h=J.data[d+3],U=E.data[d+3];h===0?(E.data[d]=255,E.data[d+1]=0):(E.data[d]=U===0?0:E.data[d],E.data[d+1]=h),E.data[d+2]=255,E.data[d+3]=255}a.putImageData(E,0,0),Y=E,e.toBlob(d=>{if(!d){c(new Error("Failed to create PNG blob"));return}f({imageData:Y,pngBlob:d})},"image/png")},t.onerror=()=>c(new Error("Failed to load image")),t.src=typeof o=="string"?o:URL.createObjectURL(o)})}function _o(o,e,a,i,f,c){const n=a.length,t=new Int32Array(n*4);for(let m=0;m<n;m++){const s=a[m],u=s%f,p=Math.floor(s/f);t[m*4+0]=u<f-1&&o[s+1]?s+1:-1,t[m*4+1]=u>0&&o[s-1]?s-1:-1,t[m*4+2]=p>0&&o[s-f]?s-f:-1,t[m*4+3]=p<c-1&&o[s+f]?s+f:-1}return{interiorPixels:a,boundaryPixels:i,pixelCount:n,neighborIndices:t}}function yo(o,e,a,i,f){const c=be.iterations,n=.01,t=new Float32Array(i*f),{interiorPixels:m,neighborIndices:s,pixelCount:u}=o;performance.now();const p=1.9,b=[],k=[];for(let y=0;y<u;y++){const r=m[y],l=r%i,v=Math.floor(r/i);(l+v)%2===0?b.push(y):k.push(y)}for(let y=0;y<c;y++){for(const r of b){const l=m[r],v=s[r*4+0],C=s[r*4+1],S=s[r*4+2],B=s[r*4+3];let g=0;v>=0&&(g+=t[v]),C>=0&&(g+=t[C]),S>=0&&(g+=t[S]),B>=0&&(g+=t[B]);const A=(n+g)/4;t[l]=p*A+(1-p)*t[l]}for(const r of k){const l=m[r],v=s[r*4+0],C=s[r*4+1],S=s[r*4+2],B=s[r*4+3];let g=0;v>=0&&(g+=t[v]),C>=0&&(g+=t[C]),S>=0&&(g+=t[S]),B>=0&&(g+=t[B]);const A=(n+g)/4;t[l]=p*A+(1-p)*t[l]}}return t}const Ao={none:0,circle:1,daisy:2,diamond:3,metaballs:4},bo=`#version 300 es
precision mediump float;
...
`,Co={classic:0,gooey:1,holes:2,soft:3},wo={square:0,hex:1},Bo=`#version 300 es
precision mediump float;
...
`,So={dots:0,ink:1,sharp:2},xe={maxColorCount:6},Uo=`#version 300 es
precision mediump float;
