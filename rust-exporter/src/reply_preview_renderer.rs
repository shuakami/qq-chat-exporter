//! 被引用消息（reply 卡片）预览元素渲染（对应 TS `replyPreviewRenderer.ts`，
//! issue #128 子项汇总）。
//!
//! 解析阶段会把 referencedMessage 的每个子元素塞进 `previewElements`，类型
//! 可能是 text / image / face / marketFace / video / audio / file。本模块只
//! 负责字符串拼接，不持有 IO 状态，行为靠注入的 `ReplyPreviewRenderContext`
//! 控制。
