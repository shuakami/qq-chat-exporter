//! 基础导出器共享功能（对应 TS `BaseExporter`）。
//!
//! Rust 侧不采用继承，而是把 TS 抽象基类拆分为：
//! - `ExporterContext`：格式、选项、进度、取消令牌等共享状态；
//! - 独立的纯函数工具（`escape_html`、时间格式化、消息排序等）。
//!
//! 并发与安全：进度回调为 `Arc<dyn Fn + Send + Sync>`，取消标志为原子布尔，
//! 无锁、无数据竞争；所有 I/O 走 tokio 异步接口并显式处理错误。
