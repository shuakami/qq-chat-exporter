//! chunked-jsonl 写入器（对应 TS `ChunkedJsonlWriter.ts`）。
//!
//! 把 JSONL 数据按消息数 / 字节数切分为多个 chunk 文件，并记录每个 chunk 的
//! 元信息（起止时间 / 条数 / 字节数 / 相对路径）。写入走 `BufferedTextWriter`，
//! 全程 backpressure 友好、内存占用恒定。
