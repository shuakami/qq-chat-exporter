//! 现代化 HTML 导出器（对应 TS `ModernHtmlExporter.ts`）。
//!
//! 两种导出模式：
//! - **单文件流式**：`export`。文档头 → 逐条消息 → 文档尾，写入全程遵循
//!   backpressure（tokio `BufWriter` + 定期 flush），资源以受限并发复制到
//!   `resources/<type>/` 目录；Issue #311 的自包含模式改为把资源读成
//!   base64 data URI 直接内联进 HTML。
//! - **Chunked viewer**：`export_chunked`（Issue #467）。输出
//!   `index.html + assets/ + data/manifest(.js/.json) + data/chunks/*.js +
//!   data/index/msgid_bXX.js`，chunk 级 Bloom 全文索引 + msgId 分桶索引，
//!   支持前端虚拟滚动与按需加载。
//!
//! 并发与安全：资源复制通过 `tokio::task::JoinSet` 限制在 `[2, 8]` 并发，
//! 单个资源失败仅记录跳过、绝不中断导出；所有共享状态归导出器独占
//! （`&mut self`），无锁、无数据竞争。
