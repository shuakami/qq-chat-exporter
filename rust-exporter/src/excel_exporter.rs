//! Excel 格式导出器（对应 TS `ExcelExporter.ts`）。
//!
//! 生成含最多 4 个工作表的 .xlsx 文件：聊天记录 / 统计信息 / 发送者统计 /
//! 资源列表。TS 侧的 `SimpleMessageParser.calculateStatistics` 语义在本模块
//! 内以 `calculate_statistics` 完整复刻（bySender 以显示名为键、insertion order
//! 保序、`Math.ceil` 时间跨度等）。
//!
//! Rust 侧使用 `rust_xlsxwriter`（纯 Rust、无 unsafe），列宽与 TS 的
//! `!cols wch` 配置一致。
