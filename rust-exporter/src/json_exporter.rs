//! JSON 格式导出器（对应 TS `JsonExporter.ts`）。
//!
//! 双模式：
//! - `single-json`：单文件 JSON，两阶段流式（阶段 1 边解析边写 NDJSON 临时文件，
//!   阶段 2 流式合成最终 JSON），全程不构建全量字符串；
//! - `chunked-jsonl`：manifest.json + chunks/*.jsonl（可选 avatars.json），
//!   按消息数 / 字节数切分，绝对流式。
//!
//! 解析层（MessageParser / SimpleMessageParser）在 Rust 侧由调用方完成，本模块
//! 消费 `CleanMessage`；issue #277 的资源路径覆写、issue #192 的临时文件放输出
//! 目录、头像 base64 预下载等行为均与 TS 对齐。
