//! 纯文本导出器（对应 TS `TextExporter.ts`）。
//!
//! TS 侧的双解析器策略（MessageParser → SimpleMessageParser fallback）属于解析层，
//! Rust 导出器直接消费解析完成的 `CleanMessage`，格式化逻辑与 TS fallback 路径
//! （CleanMessage → ParsedMessage → formatMessage）逐行对齐。
