//! 核心数据模型。
//!
//! 与 TypeScript 侧的 `CleanMessage`（SimpleMessageParser 输出）保持 100% 字段兼容：
//! 序列化后的 JSON 结构与 TS 版一致（camelCase、可选字段缺省时省略）。
//!
//! 设计说明：
//! - TS 的 `null / undefined` 统一映射为 `Option<T>`；
//! - 消息元素 `elements[].data` 保留为 `serde_json::Value`（TS 侧同样是弱类型透传，
//!   任何强类型化都会破坏与既有导出文件的字节级兼容）；
//! - 所有结构实现 `Clone`，但导出流程按引用传递，不做全量克隆。
