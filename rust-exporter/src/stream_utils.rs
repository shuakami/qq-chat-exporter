//! 流式写入工具（对应 TS `JsonStreamUtils.ts`）。
//!
//! Node 的写流靠 `drain` 事件做 backpressure；tokio 的 `AsyncWrite` 语义天然
//! backpressure-aware（`write_all` 在内核缓冲满时挂起），因此这里只需要保留
//! 「小缓冲聚合写」的优化：`BufferedTextWriter` 攒够阈值再触发一次系统调用，
//! 缓冲大小恒定可控，绝不无界增长。
