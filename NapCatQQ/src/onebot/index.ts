...
import { plugin_init, plugin_onmessage, plugin_onevent, plugin_cleanup } from '@/plugin';
...
export class NapCatOneBot11Adapter {
    ...
    private internalPluginInstance: OB11PluginMangerAdapter | null = null;
    ...
    async InitOneBot() {
        ...
        // 注册内置插件
        await this.initializeInternalPlugin();
     ...
     * 初始化内置插件
     */
    private async initializeInternalPlugin(): Promise<void> {
        try {
            this.context.logger.log('[Internal Plugin] 正在初始化内置插件...');
            
            // 创建一个虚拟的插件管理器实例用于内置插件
            this.internalPluginInstance = new OB11PluginMangerAdapter('internal_plugin', this.core, this, this.actions);
            
            // 调用内置插件的初始化函数
            await plugin_init(this.core, this, this.actions, this.internalPluginInstance);
            
            this.context.logger.log('[Internal Plugin] 内置插件初始化成功');
        } catch (error) {
            this.context.logger.logError('[Internal Plugin] 内置插件初始化失败:', error);
        }
    }
     ...
     * 处理内置插件消息
     */
    private async handleInternalPluginMessage(ob11Msg: { stringMsg: OB11Message; arrayMsg: OB11Message }, isSelfMsg: boolean): Promise<void> {
        if (!this.internalPluginInstance) {
            return;
        }

        try {
            // 使用数组格式的消息调用插件
            const message = ob11Msg.arrayMsg;
            
            // 调用插件的消息处理方法
            await plugin_onmessage('internal_plugin', this.core, this, message, this.actions, this.internalPluginInstance);
            
        } catch (error) {
            this.context.logger.logError('[Internal Plugin] 消息处理错误:', error);
        }
    }
    ...
    private async handleMsg(message: RawMessage, network: Array<NetworkAdapterConfig>) {
        ...
        try {
            ...
            if (ob11Msg) {
                ...
                // 调用内置插件的消息处理
                await this.handleInternalPluginMessage(ob11Msg, isSelfMsg);
            }
