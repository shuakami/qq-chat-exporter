...
export class NTQQFileApi {
    ...
    async downloadMedia(msgId: string, chatType: ChatType, peerUid: string, elementId: string, thumbPath: string, sourcePath: string, timeout = 1000 * 60 * 2, force: boolean = false) {
        // 用于下载文件
        console.log(`[FileApi] downloadMedia 调用参数:`, {
            msgId, chatType, peerUid, elementId, thumbPath, sourcePath, timeout, force
        });
        
        if (sourcePath && fs.existsSync(sourcePath)) {
            if (force) {
                try {
                    console.log(`[FileApi] 强制删除已存在文件: ${sourcePath}`);
                    await fsPromises.unlink(sourcePath);
                } catch {
                    //
                }
            } else {
                console.log(`[FileApi] 文件已存在，直接返回: ${sourcePath}`);
                return sourcePath;
            }
        }
        
        console.log(`[FileApi] 调用底层下载API...`);
        ...
        console.log(`[FileApi] 底层API返回数据:`, {
            filePath: completeRetData.filePath,
            msgId: completeRetData.msgId,
            elementId: completeRetData.msgElementId,
            fileDownType: completeRetData.fileDownType,
            totalSize: completeRetData.totalSize,
            fileProgress: completeRetData.fileProgress
        });
        
        const resultPath = completeRetData.filePath;
        if (!resultPath || resultPath.trim() === '') {
            console.error(`[FileApi] 警告: 底层API返回空路径，原始数据:`, completeRetData);
        } else {
            console.log(`[FileApi] 下载成功，路径: ${resultPath}`);
        }
