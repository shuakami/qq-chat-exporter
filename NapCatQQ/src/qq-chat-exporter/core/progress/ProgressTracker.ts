 ...
 * 预定义的任务阶段
 */
const DEFAULT_PHASES: TaskPhase[] = [
...
export class ProgressTracker extends EventEmitter {
    private readonly taskStates: Map<string, ExportTaskState> = new Map();
    private readonly taskConfigs: Map<string, any> = new Map(); // 存储任务配置以支持自动保存
    private readonly progressSnapshots: Map<string, ProgressSnapshot[]> = new Map();
    private readonly performanceStats: Map<string, PerformanceStats> = new Map();
    private readonly taskPhases: Map<string, TaskPhase[]> = new Map();
    ...
    async initializeTask(
        ...
        phases: TaskPhase[] = DEFAULT_PHASES
    ): Promise<void> {
        try {
            // 检查是否已存在任务状态（断点续传）
            const existingTask = await this.dbManager.loadTask(taskId);
            
            let taskState: ExportTaskState;
    ...
    updateProgress(
        ...
        if (successCount !== undefined) taskState.successCount = successCount;
        if (failureCount !== undefined) taskState.failureCount = failureCount;
        if (currentMessageId !== undefined) taskState.currentMessageId = currentMessageId;
     ...
     * 计算处理速度
     */
    private calculateProcessingSpeed(taskId: string): void {
        const taskState = this.taskStates.get(taskId);
        const snapshots = this.progressSnapshots.get(taskId);
        if (!taskState || !snapshots || snapshots.length === 0) return;
     ...
     * 估计剩余时间
     */
    private estimateRemainingTime(taskId: string): void {
        const taskState = this.taskStates.get(taskId);
        if (!taskState || !taskState.processingSpeed || taskState.processingSpeed <= 0) return;
     ...
     * 更新阶段进度
     */
    private updatePhaseProgress(taskId: string): void {
        const phases = this.taskPhases.get(taskId);
        if (!phases) return;
     ...
     * 计算总体进度
     */
    private calculateOverallProgress(taskId: string): number {
        const taskState = this.taskStates.get(taskId);
        const phases = this.taskPhases.get(taskId);
        
        if (!taskState || !phases) return 0;
     ...
     * 记录进度快照
     */
    private recordProgressSnapshot(taskId: string): void {
        const taskState = this.taskStates.get(taskId);
        const phases = this.taskPhases.get(taskId);
        if (!taskState) return;
     ...
     * 启动性能监控
     */
    private startPerformanceMonitoring(taskId: string): void {
     ...
     * 停止性能监控
     */
    private stopPerformanceMonitoring(taskId: string): void {
    ...
    getActiveTasks(): ExportTaskState[] {
        return Array.from(this.taskStates.values()).filter(
            state => state.status === ExportTaskStatus.RUNNING || 
