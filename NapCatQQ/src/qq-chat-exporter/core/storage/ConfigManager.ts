...
export class ConfigManager {
    ...
    /** 配置变更监听器 */
    private configChangeListeners: Array<(config: SystemConfig & UserConfig) => void> = [];
    ...
    private async loadSystemConfig(): Promise<void> {
        if (fs.existsSync(this.systemConfigPath)) {
            try {
                ...
                const loadedConfig = JSON.parse(configData) as Partial<SystemConfig>;
    ...
    private async loadUserConfig(): Promise<void> {
        if (fs.existsSync(this.userConfigPath)) {
            try {
                ...
                const loadedConfig = JSON.parse(configData) as Partial<UserConfig>;
    ...
    private applyEnvironmentOverrides(): void {
        const envMappings: Array<[string, keyof SystemConfig, (value: string) => any]> = [
        ...
        envMappings.forEach(([envVar, configKey, converter]) => {
            ...
            if (value !== undefined) {
                try {
                    (this.systemConfig as any)[configKey] = converter(value);
                } catch (error) {
    ...
    private validateConfig(): void {
        const systemValidationRules: Partial<Record<keyof SystemConfig, ConfigValidationRule<any>>> = {
            defaultBatchSize: {
        ...
        Object.entries(systemValidationRules).forEach(([key, rule]) => {
            const value = (this.systemConfig as any)[key];
     ...
     * 处理配置文件变更
     */
    private async handleConfigFileChange(configType: 'system' | 'user'): Promise<void> {
        try {
            // 防抖处理，避免频繁重载
     ...
     * 确保必要的目录存在
     */
    private ensureRequiredDirectoriesExist(): void {
        const directories = [
