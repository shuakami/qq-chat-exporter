...
import { ScheduledExportConfig, ExecutionHistory } from '../scheduler/ScheduledExportManager';
...
interface MemoryIndexes {
    tasks: Map<string, TaskDbRecord>;
    messages: Map<string, Map<string, MessageDbRecord>>; // taskId -> messageId -> record
    resources: Map<string, ResourceInfo>; // md5 -> resource
    systemInfo: Map<string, any>;
    scheduledExports: Map<string, ScheduledExportConfig>; // scheduledExportId -> config
...
export class DatabaseManager {
    ...
    private readonly files: JsonlFiles;
    ...
    private async loadTaskIndex(): Promise<void> {
        ...
        for await (const line of rl) {
            ...
            if (line.trim()) {
                try {
                    const task = JSON.parse(line) as TaskDbRecord;
    ...
    private async loadMessageIndex(): Promise<void> {
        if (!fs.existsSync(this.files.messages)) return;
        ...
        for await (const line of rl) {
            if (line.trim()) {
                try {
                    const message = JSON.parse(line) as MessageDbRecord;
    ...
    private async loadResourceIndex(): Promise<void> {
        if (!fs.existsSync(this.files.resources)) return;
     ...
     * 加载系统信息索引
     */
    private async loadSystemInfoIndex(): Promise<void> {
        if (!fs.existsSync(this.files.systemInfo)) return;
        ...
        for await (const line of rl) {
            if (line.trim()) {
                try {
                    const info = JSON.parse(line) as { key: string; value: any; updated_at: string };
     ...
     * 添加数据到写入队列
     */
    private queueWrite(file: string, data: any): void {
     ...
     * 调度延时写入
     */
    private scheduleDelayedWrite(): void {
     ...
     * 加载任务配置和状态
     */
    async loadTask(taskId: string): Promise<{ config: ExportTaskConfig, state: ExportTaskState } | null> {
        this.ensureInitialized();
     ...
     * 获取所有任务
     */
    async getAllTasks(): Promise<Array<{ config: ExportTaskConfig, state: ExportTaskState }>> {
        this.ensureInitialized();

        console.info(`[DatabaseManager] getAllTasks 调用，内存中有 ${this.indexes.tasks.size} 个任务`);
        const results: Array<{ config: ExportTaskConfig, state: ExportTaskState }> = [];
     ...
     * 重建消息文件
     */
    private async rebuildMessageFile(): Promise<void> {
        const allMessages: MessageDbRecord[] = [];
     ...
     * 获取未处理的消息
     */
    async getUnprocessedMessages(taskId: string): Promise<any[]> {
        this.ensureInitialized();
        ...
        // 按消息时间排序
        return unprocessedMessages.sort((a, b) => 
            parseInt(a.msgTime) - parseInt(b.msgTime)
        );
    }
     ...
     * 获取任务进度统计
     */
    async getTaskProgress(taskId: string): Promise<{ total: number, processed: number }> {
        this.ensureInitialized();
     ...
     * 保存资源信息（遗留方法，保持向后兼容）
     */
    async saveResources(_taskId: string, _messageId: string, resources: any[]): Promise<void> {
        this.ensureInitialized();

        // 这个方法现在委托给 saveResourceInfo
        for (const resource of resources) {
            if (resource.md5) {
                await this.saveResourceInfo(resource as ResourceInfo);
            }
        }
    }
     ...
     */
    async getTaskResources(_taskId: string): Promise<any[]> {
        this.ensureInitialized();
     ...
     * 设置系统信息
     */
    private setSystemInfo(key: string, value: string): void {
        const systemInfo = {
            key,
            value,
     ...
     * 根据MD5获取资源信息
     */
    async getResourceByMd5(md5: string): Promise<ResourceInfo | null> {
        this.ensureInitialized();
     ...
     * 根据状态获取资源列表
     */
    async getResourcesByStatus(status: ResourceStatus): Promise<ResourceInfo[]> {
        this.ensureInitialized();
     ...
     * 获取早于指定时间的资源
     */
    async getResourcesOlderThan(cutoffTime: Date): Promise<ResourceInfo[]> {
        this.ensureInitialized();
     ...
     * 加载定时导出任务索引
     */
    private async loadScheduledExportIndex(): Promise<void> {
        ...
        for await (const line of rl) {
            if (line.trim()) {
                try {
                    const scheduledExport: ScheduledExportConfig = JSON.parse(line);
                    // 处理日期字段
     ...
     * 加载执行历史索引
     */
    private async loadExecutionHistoryIndex(): Promise<void> {
        ...
        for await (const line of rl) {
            if (line.trim()) {
                try {
                    const history: ExecutionHistory = JSON.parse(line);
                    // 处理日期字段
     ...
     * 保存定时导出任务
     */
    async saveScheduledExport(scheduledExport: ScheduledExportConfig): Promise<void> {
        this.ensureInitialized();
    ...
    async getScheduledExports(): Promise<ScheduledExportConfig[]> {
        ...
        return Array.from(this.indexes.scheduledExports.values()).sort((a, b) => 
     ...
     * 重建定时导出任务文件
     */
    private async rebuildScheduledExportFile(): Promise<void> {
     ...
     * 重建执行历史文件
     */
    private async rebuildExecutionHistoryFile(): Promise<void> {
