//! 导出文件 / 资源管理路由（对应 TS ApiServer 3156-3520 行与 ResourceMerger）：
//! `/api/exports/files*`、`/api/resources/*`、`/api/download-file`、
//! `/api/open-file-location`、`/api/open-export-directory`、`/api/merge-resources*`。
...
pub async fn export_file_resource(
    State(state): State<SharedState>,
    Extension(RequestId(request_id)): Extension<RequestId>,
    Path((_file_name, resource_path)): Path<(String, String)>,
) -> Response {
    // 安全检查：防止路径遍历。
    ...
    let Some(full_path) = find_resource_file(&state, &resource_path).await else {
