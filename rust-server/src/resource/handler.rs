//! 资源处理器。
//!
//! 与 TS 侧 `ResourceHandler.ts` 语义对齐：负责资源的下载、健康检查、缓存管理
//! 与熔断机制，支持图片 / 视频 / 音频 / 文件四类资源。
//!
//! 并发模型：TS 侧使用轮询队列 + activeDownloads Map 控制并发；Rust 侧改用
//! `tokio::sync::Semaphore` + `JoinSet`，语义等价（优先级排序、并发上限、指数
//! 退避重试、熔断）且无轮询开销。实际下载经由 [`MediaDownloader`] trait 抽象，
//! 由 NapCat bridge 客户端实现（对应 TS `core.apis.FileApi.downloadMedia`）。
