//! 高性能 JSONL 数据库管理器。
//!
//! 与 TS 侧 `DatabaseManager.ts` 语义对齐：使用 JSON Lines 追加写 + 全量内存索引，
//! 数据文件（tasks.jsonl / resources.jsonl / system_info.jsonl / scheduled_exports.jsonl /
//! execution_history.jsonl）与 TS 版本完全兼容，可无缝读取既有数据目录。
//!
//! 并发模型：所有内存索引集中在 [`DbState`] 内，由一把 `tokio::sync::Mutex` 保护
//! （锁范围只覆盖内存操作，文件 I/O 在拷贝出数据后进行）。写入经由队列批量落盘，
//! 由后台 flush 任务每 100ms 冲刷一次，与 TS 的 `setupBatchWrite` 行为一致。
