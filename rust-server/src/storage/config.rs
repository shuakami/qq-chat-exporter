//! 配置管理器。
//!
//! 与 TS 侧 `ConfigManager.ts` 语义对齐：系统配置（config.json）+ 用户配置
//! （user-config.json），支持默认值合并、环境变量覆盖、配置验证与热重载。
//!
//! 热重载：TS 使用 `fs.watch`；Rust 侧为避免引入平台相关依赖，采用 mtime 轮询
//! （1s 间隔）实现等价行为，变更后重新加载、验证并通过 `tokio::sync::watch`
//! 通道广播新配置。
