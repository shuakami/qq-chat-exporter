...
//!
//! 当合并转发消息卡片无法通过 getMultiMsg 拉到子消息时（消息已过期 / NapCat 拒绝下发 /
//! 嵌套过深），旧版会把整段 XML 当作 summary 直接落进导出文件。这里用纯 regex 抠 QQ
//! 客户端协议规定的固定结构（不引入 xml 依赖），抠出 header / preview 行 / 末尾 summary。
//!
//! 与 TS `multiForwardXmlParser.ts` 逐行对齐。
