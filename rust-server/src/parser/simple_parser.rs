//! 简化消息解析器（`SimpleMessageParser.ts` 的 Rust 移植）。
//!
//! 将 NapCat Core 返回的 `RawMessage`（经 bridge 以 JSON 传输，故此处以
//! `serde_json::Value` 表达，字段访问语义与 TS 侧动态访问一一对应）解析为
//! `CleanMessage`。包含：
//! - 发件人显示名解析（群名片 > 备注 > 昵称，#274 缓存回退）
//! - 全量消息元素解析（文本 / 表情 / 图片 / 文件 / 视频 / 语音 / 回复 /
//!   合并转发 / JSON 卡片 / 灰条系统消息等）
//! - 回复预览元素（issue #128）与合并转发子消息递归拉取（issue #161）
//! - 资源路径二次回填（issue #30 / #277）
