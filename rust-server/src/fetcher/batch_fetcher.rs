//! 批量消息获取器。
//!
//! 与 TS 侧 `BatchMessageFetcher.ts` 语义对齐：封装 NapCat 底层消息获取 API，
//! 提供带筛选、分页、重试与自适应 batchSize 缩减（issue #305 / #316）的批量
//! 获取能力。TS 的 `AsyncGenerator` 分页流在 Rust 侧映射为 `fetch_next_batch`
//! 驱动的显式分页循环（调用方 while-let 拉取），避免自引用生成器复杂度。
//!
//! NapCat API 经 [`MessageFetchApi`] trait 抽象（由 bridge 客户端实现），消息
//! 采用 `serde_json::Value`（NapCat RawMessage 原始 JSON）在解析层之前流转。
