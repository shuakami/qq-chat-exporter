//! 定时导出管理器。
//!
//! 与 TS `ScheduledExportManager.ts` 语义对齐：负责定时导出任务的 CRUD、
//! cron 调度（每分钟检查一次，Asia/Shanghai 本地时区语义）、时间范围计算、
//! 手动触发 / 一键触发全部（issue #445）、执行历史记录（issue #363 资源摘要）。
//!
//! 实际的「拉取消息 → 解析 → 导出」流程由 [`ScheduledExportExecutor`] trait
//! 抽象注入 —— 集成阶段由服务端把 fetcher / parser / exporter 组装成执行器，
//! 使调度层与 NapCat bridge 解耦、可独立测试。
