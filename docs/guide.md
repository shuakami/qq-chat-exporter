...
- **只想开箱即用，直接导出聊天记录**：下载 `NapCat-QCE-Windows-x64-vxxx.zip`
...
1. 先安装 LiteLoaderQQNT：<https://liteloaderqqnt.github.io/>
   进入 LiteLoaderQQNT 的发布页后，只下载 `LiteLoaderQQNT.zip`。
   **不要下载 `Source code`。**
2. 再回到 QCE 自己的发布页，下载 `NapCat-Framework-QCE-vxxx.zip`：
   <https://github.com/shuakami/qq-chat-exporter/releases>
3. 把 `NapCat-Framework-QCE-vxxx.zip` 解压到一个你容易找到的位置，例如 `C:\Users\你的用户名\Documents\NapCat\`
4. 将解压后的 NapCat 文件夹复制到 LiteLoader 插件目录：

   - Windows：
     目标目录是 `"%APPDATA%\\LiteLoaderQQNT\\plugins\\"`
     按 `Win + R`，输入 `"%APPDATA%\\LiteLoaderQQNT\\plugins\\"` 后回车，把解压出来的 `NapCat` 文件夹复制进去。
   - Linux：
     目标目录是 `~/.config/LiteLoaderQQNT/plugins/`
     在终端执行 `xdg-open ~/.config/LiteLoaderQQNT/plugins/`，然后把解压出来的 `NapCat` 文件夹复制进去。
   - macOS：
     目标目录是 `~/Library/Application Support/LiteLoaderQQNT/plugins/`
     在访达按 `Shift + Command + G`，输入 `~/Library/Application Support/LiteLoaderQQNT/plugins/` 后回车，把解压出来的 `NapCat` 文件夹复制进去。
5. **重要**：先完全退出已经打开的 QQ，再继续下一步
6. 运行 `napiLoader.bat`
7. 用手机扫码登录 QQ
8. 在 `security.json` 中查找 `token`
...
9. 访问 `http://localhost:40653/qce-v4-tool`，使用找到的 token 进行操作

> Framework 模式只需要下载上面提到的两个压缩包：
> 
> - LiteLoaderQQNT 的 `LiteLoaderQQNT.zip`
> - QCE 的 `NapCat-Framework-QCE-vxxx.zip`
> 
> 不需要同时下载 Windows Shell 包，也不需要下载 Linux 包。
