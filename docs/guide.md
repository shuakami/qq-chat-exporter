...
7. 访问 `http://localhost:40653/qce`，使用找到的 token 进行操作
...
- **打开界面**：浏览器访问 `http://localhost:40653/qce`，输入控制台里显示的 `Access Token`。

> 只有控制台里出现了 `Web: http://127.0.0.1:40653/qce` 和 `Token`，才说明启动成功了。  
...
1. 先看黑色控制台窗口还在不在。
2. 如果控制台里有红字报错，或者根本没有出现 `Web: http://127.0.0.1:40653/qce`，说明启动失败了，这时刷新浏览器没有用。
3. 确认你运行的是下面这些文件之一：
...
- **操作**：直接访问 `http://localhost:40653/qce`，无需 Token。
