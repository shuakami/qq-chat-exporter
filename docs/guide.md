...
普通导出只读取当前客户端已经同步到本机的记录。如果 QQ 里能看到更早的私聊、但普通导出缺失，可以在**完整模式**下尝试实验性的漫游导出：

1. 联系人仍在会话列表中时，点击该行“导出”右侧的下拉箭头，再选择 **“漫游导出”**。
2. 联系人不在列表中时，在会话页搜索框输入对方 QQ 号；列表无匹配项后，使用 **“按 QQ 号反查会话”** 查询，再点击结果中的 **“漫游导出”**。
3. 选择必填的开始、结束日期和导出格式。单个任务最多扫描 1461 个本机日历日，更长范围请拆分为多个任务。
4. 在任务页查看逐日扫描进度，或随时停止任务。带有 **“结果可能不完整”** 标记的任务只能代表当前账号和客户端实际返回的内容。

该功能仅支持私聊，不支持群聊或独立查看模式；即使任务没有显示不完整，也不能证明腾讯服务端保存了全部历史。接口调用、扫描边界和错误语义见[实验性私聊漫游导出与查询 API](roaming-api.md)。
