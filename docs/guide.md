...
1. 先按 LiteLoaderQQNT 官方文档完成安装：<https://liteloaderqqnt.github.io/>
   装好以后，打开 QQ 设置，确认左侧已经出现 LiteLoaderQQNT，再继续下一步。
...
4. 这个 Framework 包已经把需要的内容整合好了，普通用户直接解压后继续往下做就可以，不需要再手动挪文件夹。
5. **重要**：先完全退出已经打开的 QQ，再继续下一步。
6. 在解压后的目录里运行 `napiLoader.bat`
7. 如果 QQ 弹出登录页，按平时的方式登录 QQ。
...
> Framework 模式先要把 LiteLoaderQQNT 安装好，再下载 QCE 的 `NapCat-Framework-QCE-vxxx.zip`。
> 
> 普通用户直接用这个 Framework 包继续往下做就可以，也不需要同时下载 Windows Shell 包或 Linux 包。
